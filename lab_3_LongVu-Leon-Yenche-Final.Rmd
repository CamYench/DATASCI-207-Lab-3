...
```{r Inspect data, warning=FALSE, echo=FALSE, message=FALSE}
# Preliminary EDA 

# Analyze seatbelt laws
...
> From inspection of the proportions of the one hot variables, we see that they are not true one-hot variables and there are, in fact, other intermediate values present in the data. To address this, we will have to round some of the numbers on a case by case basis to maintain a more uniform dataset to build models from. Note that the proportions of unexpected values in the data are quite low, so their manipulation (from rounding) will not significantly alter the general trends found in the data.'

```{r Data wrangling BAC}
# Unify and correct BAC limit data
data %>% mutate(
...
```{r Build and Describe data, warning=FALSE, echo=FALSE, message=FALSE}
data <- data %>% mutate(
  # Create unified speed limit variable
  ...
  # Combine year of observation data into unified variable
  ...
  # Create blood alcohol limit variable
  ...
  # Create vehicle miles traveled per capita law variable
  veh_mile_trav_percap = vehicmiles*1000000000/statepop,
  
  # Round perse law data
  perse = round(perse),
  
  # Round zero tolerance law data
...
# Plot total traffic fatalities over time by state
data %>% ggplot(aes(x = year, y = tot_fata_per_100th_pop)) + geom_line() + facet_wrap(~state_name) + ggtitle("Total Traffic Fatilities Over Years by States")

# Plot Total Traffic Fatalities by state
data %>% ggplot() + geom_boxplot(aes(y = tot_fata_per_100th_pop, x = factor(state_name))) + ggtitle("Total Traffic Fatalities by States")

# Plot Total Traffic Fatalities by year
data %>% ggplot() + geom_boxplot(aes(y = tot_fata_per_100th_pop, x = factor(year))) + ggtitle("Total Traffic Fatalities by Year")

# Plot total Traffic Fatalities by speed limit category
data %>% ggplot() + geom_boxplot(aes(y = tot_fata_per_100th_pop, x = factor(speed_limit_bin))) + ggtitle("Total Traffic Fatalitites by Speed Limit Category")

# Create a slimmer data set
...
# Plot Total Night Fatalities by state
...
# Plot Total Weekend Fatalities by state
...
# Plot Total Fatalities by Unemployment rate by state
...
# Plot Total Fatalities versus age demographics by state
...
# Plot Total Fatalities versus miles travelled
...
# Plot Total Fatalities by blood alcohol limit
data %>% ggplot() + geom_boxplot(aes(y = tot_fata_per_100th_pop, x = factor(bld_alc_lmt_cat))) + ggtitle("Total Fatalities by Blood Alcohol Limit")

# Plot Total Fatalities by blood alcohol limit by state
data %>% ggplot() + geom_boxplot(aes(y = tot_fata_per_100th_pop, x = factor(bld_alc_lmt_cat))) + ggtitle("Total Fatalities by Blood Alcohol Limit") + facet_wrap(~state_name)

# Plot Total Fatalities by per se laws
data %>% ggplot() + geom_boxplot(aes(y = tot_fata_per_100th_pop, x = factor(per_se_law))) + ggtitle("Total Fatalities based on Per Se Law")

# Plot Total Fatalities by seatbelt laws
...
# Plot Total Fatalities by zero tolerance laws
...
# Create state-level fixed effect model
...
# Conduct Durbin Watson test and Breusch-Paga Tests
pdwtest(statefe_model)
