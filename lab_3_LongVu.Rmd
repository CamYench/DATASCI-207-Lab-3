...
data <- data %>% mutate(
  speed_limit = sl55 * 55 + sl65 * 65 + sl70 * 70 + sl75 * 75 + slnone * 200,
  ...
  bld_alc_lmt = case_when((bld_alc_lmt == 0) ~ 0.5, 
                          (bld_alc_lmt > 0) ~ bld_alc_lmt),
...
pre_model <- plm(tot_fata_per_100th_pop ~ year, data = data, 
...
statefe_model <- plm(tot_fata_per_100th_pop ~ year + bld_alc_lmt +per_se_law + prim_seatbelt_law 
                     + second_seatbelt_law + speed_lim_grter70 + grad_driver_license_law +                                                    age1424_pop_percent + unemp_rate + veh_mile_trav_percap, data = data, 
                 index = c("state_name", "year"), 
