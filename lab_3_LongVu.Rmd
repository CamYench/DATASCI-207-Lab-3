...
data <- data %>% mutate(
  ...
  veh_mile_trav_percap = vehicmiles/statepop
)
...
pre_model <- plm(tot_fata_per_100th_pop ~ year, data = pdata, 
                 index = c("year", "state"), 
...
The team performed the following transformation on the raw dataset:
 - Generate a new variable to re-encode the blood-alcohol-limit columns (0.1 and 0.08). The values included in this variable is the sum product of variable `bld_alc_lim10` (named `bac10` in the raw data) with 0.10 and variable `bld_alc_lim08` (named `bac08` in the raw data) with 0.08. This new variable represents the actual blood alcohol limit in a given year (if both rules were enforced in the same year, then the value would be the weighted average blood alcohol limit of the two limits) 
...
The `per se law` has a negative slope, which means that having the law enforced reduces the total traffic fatalities by `r coef(exp_model)[27]`. This confirms with the findings of our EDA. 
...
```
The coefficient estimate for blood alcohol level changes from `r coef(exp_model)[26]` in the expanded model to `r coef(statefe_model)[73]` in the expanded model with state level fixed effect. 

The coefficient estimate for per se law changes from `r coef(exp_model)[27]` in the expanded model to `r coef(statefe_model)[74]` in the expanded model with state level fixed effect. 

The coefficient estimate for primary seat-belt law changes from `r coef(exp_model)[28]` in the expanded model to `r coef(statefe_mdoel)[75]` in the expanded model with state level fixed effect.
...
For the estimates to be consisted, the model needs to satisfy the following assumptions: 
 ...
 - Error term is homoskedastic and serially uncorrelated across time. The Durbin-Watson test shows a p-value of 0.988, which indicates that there is no serial correlation in the error term and satisfied this assumption. 
 

Although the residuals does show some level of heteroskasticity, the level is mild and thus we conclude that the homoskedasticity assumption was not violated. 
